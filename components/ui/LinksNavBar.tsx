...
export default function LinkNavBar({ menuItems }: LinkNavBarProps) {
  return (
    <div className="flex w-full flex-col gap-y-5 overflow-hidden pt-[50px] mt-auto">
      ...
      {menuItems.map((item, index) => (
        <div key={index} className="group flex justify-center gap-2 ">
          ...
          <Link href={item.href} className="text-white z-30 -translate-x-6 cursor-pointer font-mono font-semibold transition-transform duration-300 ease-out group-hover:translate-x-0 group-hover:text-blue-500 dark:text-white md:-translate-x-12 md:text-4xl md:group-hover:translate-x-0">
            {item.text}
          </Link>
